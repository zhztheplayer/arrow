...
abstract class AllocationManagerBase {
    ...
    protected BufferLedger owningLedger;
    ...
    public AllocationManagerBase(BaseAllocator accountingAllocator) {
     ...
     * @param retain Whether to increment the associated ledger's reference count
     * @return The reference manager (new or existing) that associates the underlying
     *         buffer to this new ledger.
     */
    protected BufferLedger associate(final BaseAllocator allocator, final boolean retain) {
    ...
    abstract UnsafeDirectLittleEndian getMemoryChunk();
